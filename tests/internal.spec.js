...
describe('Can resolve', function () {
  it('custom username', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email);
  ...
  it('custom registry', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email, testData.registry);
  ...
  it('custom scope', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email, testData.registry, testData.scope);
  ...
  it('custom quotes setting', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email, testData.registry, testData.scope, true);
  ...
  it('custom configuration path', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email, testData.registry, testData.scope, true, testData.configPath);
...
describe('Can append to', function () {
  ...
  it('file with existing auth token', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email, testData.registry, testData.scope);
    const toWrite = ncl.generateFileContents(args, '//npm.random.com/:_authToken=test', testData.response2);
    expect(toWrite).to.have.length(2);
...
describe('Can honour', function () {
  it('quotes setting set to false', function () {
    const args = ncl.processArguments(testData.username, testData.password, testData.email, testData.registry, testData.scope, false);
    const toWrite = ncl.generateFileContents(args, '', testData.response);
    expect(toWrite).to.have.length(2);
    expect(toWrite).to.include(`//npm.random.com/:_authToken=${testData.response.token}`);
    expect(toWrite).to.include(`${testData.scope}:registry=${testData.registry}`);
